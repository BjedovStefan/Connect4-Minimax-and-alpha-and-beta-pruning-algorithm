...
Game.prototype.place = function(column) {
    ...
    if (that.board.score() != that.score && that.board.score() != -that.score && !that.board.isFull()) {
            ...
            if (document.getElementById('game_board').rows[y].cells[column].className == 'empty') {
                if (that.round == 1) {
                    ...
                    setTimeout(function() {
                        ...
                        var laufzeit = new Date().getTime() - startzeit;
                        document.getElementById('user-time').innerHTML = laufzeit.toFixed(2) + 'ms';

                        // Place ai decision
                        that.place(ai_move[0]);

                        // Debug
                        document.getElementById('user-column').innerHTML = 'Column: ' + parseInt(ai_move[0] + 1);

                        document.getElementById('user-iterations').innerHTML = that.iterations;
        ...
        document.getElementById('ai-moves').innerHTML = 'No. moves: ' + count1;
        document.getElementById('user-moves').innerHTML = 'No. moves: ' + count2;
...
Game.prototype.updateStatus = function() {
    ...
    if (that.status == 0) {
        ...
        html.innerHTML = "Red AI won";
    } else if (that.status == 2) {
        html.className = "status-lost";
        html.innerHTML = "Yellow AI won";
    } else {
...
Game.prototype.restartGame = function() {
    if (confirm('Game is going to be restarted.\nAre you sure?')) {
        ...
        document.getElementById('ai-moves').innerHTML = "No.moves: ?";

        document.getElementById('user-iterations').innerHTML = "?";
        document.getElementById('user-time').innerHTML = "?";
        document.getElementById('user-column').innerHTML = "Column: ?";
        document.getElementById('user-moves').innerHTML = "No. moves: ?";
